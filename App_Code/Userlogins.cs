...
public class Userlogins
{
    ...
    public static Userlogins CheckUserByEmail(SqlConnection conEQ, string eMail)
    {
        Userlogins details = new Userlogins();
        try
        {
            DataTable dt = new DataTable();
            using (SqlCommand cmd = new SqlCommand("select FirstName,UserGuid,Status,EmailId from Customers where EmailId=@EmailId  and Status!='Deleted'", conEQ))
            {
    ...
    public static int CreateUser(SqlConnection conEQ, Userlogins Userlogins)
    {
        int x = 0;
        try
        {
            SqlCommand cmd = new SqlCommand("insert into Customers (UserGuid,Gender,FirstName,LastName,EmailId,ContactNo,Pwd,DOB,Status,Subscribed,ForgotId,AddedOn,AddedIp) values(@UserGuid,@Gender,@FirstName,@LastName,@EmailId,@ContactNo,@Pwd,@DOB,@Status,@Subscribed,@ForgotId,@AddedOn,@AddedIp)", conEQ);
    ...
    public static Userlogins UserLoginWithEmail(SqlConnection conEQ, string uid, string pwd)
    {
        Userlogins result = new Userlogins();
        try
        {
            DataTable dt = new DataTable();
            SqlCommand cmd = new SqlCommand("select Pwd,UserGuid,Status,FirstName from Customers where EmailId=@EmailId and Pwd=@Pwd and Status != 'Deleted'", conEQ);
    ...
    public static Userlogins UserLogin(SqlConnection conEQ, string uid, string pwd)
    {
        Userlogins result = new Userlogins();
        try
        {
    ...
    public static int UpdateAsVerifed(SqlConnection conEQ, string uGuid)
    {
        int x = 0;
        try
        {
            SqlCommand cmd1 = new SqlCommand("Update Customers Set Status='Verified' Where UserGuid=@UserGuid", conEQ);
