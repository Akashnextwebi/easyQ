$(document).ready(function () {
    ...
    $(document.body).on("click", "#btnResourceDownload", function (e1) {
        ...
        $.ajax({
            type: 'POST',
            url: 'articles.aspx/downloadResource',
...
function BindAllArticles() {
    ...
    $.ajax({
        type: 'POST',
        url: 'articles.aspx/GetAllArticles',
        ...
        success: function (res) {
            ...
            for (var i = 0; i < articles.length; i++) {
                var img = "" + articles[i].Image;
                pLength = articles[0].TotalCount;

                listings += "<div class='col-lg-4 col-md-6'> ";
                listings += "<div class='post-item wow fadeInUp'>";
                listings += "<div class='post-featured-image'>";
                listings += "<a href='javascript:void(0);'>";
                listings += "<figure class='image-anime'>";
                listings += "<img src='/" + img + "' alt='" + articles[i].Title + "' />";
                listings += "</figure>";
                listings += "</a>";
                listings += "</div>";
                listings += "<div class='post-item-body'>";
                listings += "<div class='post-item-content'>";
                listings += "<span> " +articles[i].PostedOn + "</span>";
                listings += "<h2 class='mt-2'>";
                listings += "<a href='javascript:void(0);'>" + articles[i].Article + "</a>";
                listings += "</h2>";
                listings += "</div>";
                listings += "<div class='post-item-btn'>";
                listings += "<a href='javascript:void(0)' class='readmore-btn hidenId' data-id='" + articles[i].Id + "' data-bs-toggle='modal' data-bs-target='#brochureModal'>Download Article</a>";
                listings += "</div>";
                listings += "</div>";
                listings += "</div>";
                listings += "</div>";
            ...
            $("#ArticleListBindingSec").empty();
            if (articles.length > 0) {
                $("#ArticleListBindingSec").append(listings);
                ...
                var maxHeight1 = Math.max.apply(null, $(".mainArticlePage .post-item .post__body .post__desc").map(function () {
                    return $(this).height();
                }).get());
                $(".mainArticlePage .post-item .post__body .post__desc").css("min-height", maxHeight1);
            }
...
function BindPPage(pageS, cPage, pCount) {
    ...
    //const topOffset = $("#ArticleListBindingSec").offset().top;
